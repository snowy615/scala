...
def QSort(l: Int, r: Int, a: Array[Int]): Unit = {
  if (r - l > 1) {
    val k = partition2(l, r, a)
    QSort(l, k, a)
    QSort(k + 1, r, a)
  }
}

// I don't get the tail recursive loop

/*
Q8
a) poor performance when all equal because nothing is a(i) < x, so always split into size 0 and N-1. O(N^2)
b) somewhat unbalanced split, but better than a). O(N log N)
c) means there are a lot of elements that are equal, so go into else statement. If <=, wouldn't change it but just flip the parition from the else into the if.
